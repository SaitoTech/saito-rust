...
impl Shashmap {
    ...
    pub fn insert(&mut self, slip: Slip, id: u64) {
        self.shashmap.insert(slip, id as i64);
    }
    ...
    pub fn spend_transaction(&mut self, tx: &Transaction, block_id: u64) {
        for input in tx.inputs().iter() {
            self.shashmap.insert(*input, block_id as i64);
        }
    }
    ...
    pub fn unspend_transaction(&mut self, tx: &Transaction) {
        for input in tx.inputs().iter() {
            self.shashmap.insert(*input, -1);
        }

        for outer in tx.outputs().iter() {
            self.shashmap.remove(&outer);
        }
    }
    ...
    pub fn spend_slip(&mut self, slip: &Slip, _bid: u64) {
        self.shashmap.insert(*slip, _bid as i64);
    }
...
mod test {
    ...
    fn shashmap_insert_test() {
        ...
        let slip = Slip::new(keypair.public_key().clone(), 0);
        shashmap.insert(slip, 0);
        assert!(shashmap.shashmap.contains_key(&slip));
    }
    ...
    fn shashmap_insert_new_transaction_test() {
        ...
        let keypair = Keypair::new();
        let output_slip = Slip::new(keypair.public_key().clone(), 0);

        tx.add_output(output_slip);

        shashmap.insert_new_transaction(&tx);

        assert!(shashmap.shashmap.contains_key(&output_slip));
        assert_eq!(shashmap.shashmap.get(&output_slip).unwrap(), &-1);
    }
    ...
    fn shashmap_spend_transaction_test() {
        ...
        let keypair = Keypair::new();
        let input_slip = Slip::new(keypair.public_key().clone(), 0);

        tx.add_input(input_slip);

        shashmap.spend_transaction(&tx, 0);

        assert!(shashmap.shashmap.contains_key(&input_slip));
        assert_eq!(shashmap.shashmap.get(&input_slip).unwrap(), &0);
    }
    ...
    fn shashmap_unspend_transaction_test() {
        ...
        assert!(shashmap.shashmap.contains_key(&input_slip));
        assert_eq!(shashmap.shashmap.get(&input_slip).unwrap(), &-1);
    }
