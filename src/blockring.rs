...
mod test {
	...
	block1.set_id(1);
        ...
        let mut block2 = test_manager
            .generate_block_and_metadata(block1.get_hash(), current_timestamp + 120000, 0, 1, false, vec![])
            .await;
	block2.set_id(2);

        // BLOCK 3
        let mut block3 = test_manager
            .generate_block_and_metadata(block2.get_hash(), current_timestamp + 240000, 0, 1, false, vec![])
            .await;
	block3.set_id(3);

        // BLOCK 4
        let mut block4 = test_manager
            .generate_block_and_metadata(block3.get_hash(), current_timestamp + 360000, 0, 1, false, vec![])
            .await;
	block4.set_id(4);

        // BLOCK 5
        let mut block5 = test_manager
            .generate_block_and_metadata(block4.get_hash(), current_timestamp + 480000, 0, 1, false, vec![])
            .await;
	block5.set_id(5);
	...
	Blockchain::add_block_to_blockchain(blockchain_lock.clone(), block1).await;
        ...
        // BLOCK 2
        let mut block2 = test_manager
            .generate_block_and_metadata(block1_hash, current_timestamp + 120000, 0, 1, false, vec![])
            .await;
	...
	Blockchain::add_block_to_blockchain(blockchain_lock.clone(), block2).await;
        ...
        // BLOCK 3
        let mut block3 = test_manager
            .generate_block_and_metadata(block2_hash, current_timestamp + 240000, 0, 1, false, vec![])
            .await;
	...
	Blockchain::add_block_to_blockchain(blockchain_lock.clone(), block3).await;
        ...
        // BLOCK 4
        let mut block4 = test_manager
            .generate_block_and_metadata(block3_hash, current_timestamp + 360000, 0, 1, false, vec![])
            .await;
	...
	Blockchain::add_block_to_blockchain(blockchain_lock.clone(), block4).await;
        ...
        // BLOCK 5
        let mut block5 = test_manager
            .generate_block_and_metadata(block4_hash, current_timestamp + 480000, 0, 1, false, vec![])
            .await;
	...
	let mut blockchain = blockchain_lock.write().await;
        ...
        // do we contain these block hashes?
        assert_eq!(
            blockchain.blockring.contains_block_hash_at_block_id(1, block1_hash),
            true
        );
        assert_eq!(
            blockchain.blockring.contains_block_hash_at_block_id(2, block2_hash),
            true
        );
        assert_eq!(
            blockchain.blockring.contains_block_hash_at_block_id(3, block3_hash),
            true
        );
        assert_eq!(
            blockchain.blockring.contains_block_hash_at_block_id(4, block4_hash),
            true
        );
        assert_eq!(
            blockchain.blockring.contains_block_hash_at_block_id(5, block5_hash),
            true
        );
        assert_eq!(
            blockchain.blockring.contains_block_hash_at_block_id(2, block4_hash),
            false
        );
        ...
        // reorg in the wrong block_id location, should not change
        blockchain.blockring.on_chain_reorganization(532, block5_hash, false);
        assert_ne!(blockchain.blockring.get_latest_block_id(), 2);

        blockchain.blockring.on_chain_reorganization(5, block5_hash, false);
	...
	Blockchain::add_block_to_blockchain(blockchain_lock.clone(), block2).await;
        ...
        assert_eq!([0; 32], blockring.get_longest_chain_block_hash_by_block_id(0));
        ...
        assert_eq!([0; 32], blockring.get_longest_chain_block_hash_by_block_id(3));
