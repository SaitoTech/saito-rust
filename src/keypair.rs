...
impl Keypair {
    ...
    fn new_from_private_key(private_key: &[u8]) -> Keypair {
    ...
    fn make_message_from_string(message_string: &str) -> [u8; 32] {
        ...
        hashvalue.as_slice().try_into().expect("Wrong length")
    }
...
mod test {
    ...
    fn test_signing() {
        ...
        hex::decode_to_slice(sig_string2, &mut sig_bytes as &mut [u8])
            .expect("Test did not run correctly");
    ...
    fn test_new_from_private_key() {
        ...
        write!(&mut pubkey_string, "{:?}", keypair.get_public_key()).expect("Test does not run");
        ...
        let result = Keypair::new_from_private_key_hex(
        ...
        let result = Keypair::new_from_private_key_hex("1234");
        assert_eq!(Err(FromHexError::InvalidStringLength), result);
        let result = Keypair::new_from_private_key_hex("12345");
        assert_eq!(Err(FromHexError::OddLength), result);
    }
    #[test]
    fn test_new() {
