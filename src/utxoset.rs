...
impl UTXOSet {
    ...
    pub fn insert_transaction(&mut self, tx: &Transaction, block_id: u64) {
        for (index, input_slip) in tx.signed_tx.body.inputs().iter().enumerate() {
            self.utxo_hashmap.remove(&input_slip);
        }
        for (index, _output_slip) in tx.signed_tx.body.outputs().iter().enumerate() {
            let output_as_input_slip = InputSlip::new(block_id, tx.id(), index as u64);
...
mod test {
    ...
    fn utxoset_insert_transaction_test() {
        ...
        let output_slip = OutputSlip::new(keypair.public_key().clone(), SlipBroadcastType::Normal, 10);
        let mock_block_number: u64 = 1;
        let mock_tx_number: u64 = 1;
        let input_slip = InputSlip::new(mock_block_number, mock_tx_number, 0);
