...
mod tests {
	...
	{
	    let mut mempool = mempool_lock.write().await;
	    let block1 = test_manager.generate_block([0; 32], current_timestamp+HEARTBEAT, 10, 0, false, vec![]).await;
	    block1_hash = block1.get_hash();
	    mempool.add_block_to_queue(block1);

	    let block2 = test_manager.generate_block(block1_hash, current_timestamp+120000, 0, 0, false, vec![]).await;
	    block2_hash = block2.get_hash();
	    mempool.add_block_to_queue(block2);

	    let block3 = test_manager.generate_block(block2_hash, current_timestamp+HEARTBEAT+200, 0, 0, false, vec![]).await;
...
println!("latest block 1 hash: {:?} vs {:?}", blockchain.get_latest_block_hash(), block1_hash);
println!("latest block 2 hash: {:?} vs {:?}", blockchain.get_latest_block_hash(), block2_hash);
println!("latest block 3 hash: {:?} vs {:?}", blockchain.get_latest_block_hash(), block3_hash);
